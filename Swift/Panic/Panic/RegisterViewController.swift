...
class RegisterViewController: UIViewController, UITextFieldDelegate {
    ...
    func validation() -> Bool {
		...
		// Name
        if countElements(containerView.txtName.text.trim()) < 5 { message = "Please fill in your full name\n" }
        ...
        if countElements(containerView.txtUsername.text.trim()) < 5 { message = message + "Username must be at least 5 characters\n" }
		
		// Cell number
        if countElements(containerView.txtCellNumber.text.trim()) < 10 { message = message + "Please fill in your cell number\n" }
        ...
        if !containerView.txtPassword.text.trim().isEmpty && !containerView.txtConfirmPassword.text.isEmpty {
            ...
            } else if countElements(containerView.txtPassword.text.trim()) < 6 {
