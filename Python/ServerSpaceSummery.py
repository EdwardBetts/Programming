...
output = open(save_path+'/Server_Space_Summary.csv', 'w')
output.close()
...
def connect(host, user, password, command):
	try:
		...
		output = open(save_path+'/Server_Space_Summary.csv', 'a')
		...
		if temp[:temp.find('/dev/mapper')] != -1:
			temp = temp[:temp.find('/dev/mapper')]

		#re.sub(r'/dev*',r'\n/dev*', temp)
		temp = temp[temp.find('/dev'):-1]
		...
		if len(dataList) > 6:
			...
			output.write(hostname + ',' + temp[:-1] + ',' + host)
			# print hostname + '\t' + '             ' + temp[temp.find('/dev'):-2] + '\t' + host
			print hostname + ',' + temp[:-1] + ',' + host
	...
	for item in hosts:
		if item in arena:
			if item == '192.168.0.20':
				connect(item, 'root', password, 'df -h /Archive* -P')
			elif item == '192.168.0.203':
				connect(item, 'root', password, 'df -h /Arena1/* -P')
			elif item == '192.168.0.205':
				connect(item, 'root', password, 'df -h -P --type=ext3 /arena3/raid*')
			else:
				connect(item, 'root', password, 'df -h /Arena* -P')
		if item in QNAP:
			connect(item, 'admin', password, 'df /dev/md0')
